...
class AIService {
  ...
  private scoreLoanPurpose(purpose: string): number {
    const lowRiskPurposes = [
      "business expansion",
      "education",
      "home improvement",
      "debt consolidation",
    ];
    const mediumRiskPurposes = [
      "investment",
      "equipment purchase",
      "working capital",
    ];
  ...
  private generateRecommendations(factors: any, score: number): string[] {
    ...
    if (factors.creditHistory < 70) {
      recommendations.push(
        "Improve credit history with smaller, successful loans",
      );
    }
    if (factors.collateralRatio < 50) {
      recommendations.push(
        "Consider increasing collateral ratio for better terms",
      );
    }
  ...
  async getLenderRecommendations(lenderProfile: {
    ...
    try {
      ...
      return mockBorrowers.filter((borrower) => {
        if (
          lenderProfile.riskTolerance === "low" &&
          borrower.riskLevel !== "low"
        )
          return false;
        if (
          lenderProfile.riskTolerance === "medium" &&
